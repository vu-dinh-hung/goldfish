...
pub fn pull(url: &str) {
    ...
    if try_clone(url).is_some() {
        ...
        remove_dir_contents(&og_dir);
        networking::rsync(url, ".dvcs");
        match Repository::find(".dvcs") {
            Some(repo) => {
                match repo.read_head() {
                    Ok(head_id) => {
                        match Commit::get(&repo, head_id.as_str()) {
...
fn try_clone(url: &str) -> Option<bool> {
    create_dir(".dvcs_temp");
    std::env::set_current_dir(format!("{}{}", std::env::current_dir().unwrap().display(), "/.dvcs_temp"));
    let download_succeeded = networking::rsync(url, ".dvcs");
    if download_succeeded {
        match Repository::find(".dvcs") {
            Some(repo) => {
                match repo.read_head() {
